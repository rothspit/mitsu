...
const PAGE_TITLE = '市川の出勤情報'
const PAGE_PATH = '/ichikawa'
const DISPATCH_NOTE = '西船橋店より出張対応いたします。別途交通費2,000円がかかります。'
...
// ============================================
// メインページ（出張エリア — area_idフィルタなし）
// ============================================

type ViewMode = 'week' | 'month'

export default function IchikawaSchedulePage() {
  ...
  // 出張エリア: area_id フィルタなし（全エリアの出勤を表示）
  ...
  useEffect(() => {
    if (viewMode === 'week') {
      const url = selectedDate === today ? PAGE_PATH : `${PAGE_PATH}?date=${selectedDate}`
  ...
  return (
    <main className="min-h-screen bg-[#fafaf9] text-[#1c1917] pb-24">
      ...
      {/* 出張エリア案内 */}
      <div className="bg-[#b8860b]/5 border-b border-[#b8860b]/15">
      ...
      {viewMode === 'week' && (
        <div className="max-w-2xl mx-auto px-4 pt-6 pb-4">
          <div className="text-center mb-6">
            <p className="text-xs text-[#78716c] tracking-wider">{fullDateLabel(selectedDate)}</p>
            {!loading && (
              <p className="text-2xl font-medium text-[#b8860b] mt-2 tracking-wider" style={{ fontFamily: serif }}>
                {selectedDate === today ? '本日' : ''}の出勤 <span className="text-3xl">{schedules.length}</span>名
              </p>
            )}
          </div>
          {loading ? (
            ...
            <div className="grid grid-cols-2 gap-3">
              {schedules.map((s) => (<CastCard key={s.id} schedule={s} />))}
            </div>
          ) : (
            <div className="text-center py-16">
      ...
      {viewMode === 'month' && (
        <div className="max-w-2xl mx-auto px-4 pt-6 pb-4">
          <div className="flex items-center justify-between mb-4">
            <button onClick={() => goMonth(-1)} className="text-xs text-[#78716c] hover:text-[#b8860b] transition px-3 py-1">◀ 前月</button>
            <p className="text-lg font-medium text-[#1c1917] tracking-wider" style={{ fontFamily: serif }}>{calYear}年{calMonth}月</p>
            <button onClick={() => goMonth(1)} className="text-xs text-[#78716c] hover:text-[#b8860b] transition px-3 py-1">次月 ▶</button>
          </div>
          <div className="grid grid-cols-7 mb-1">
            {['月', '火', '水', '木', '金', '土', '日'].map((label, i) => (
              <div key={label} className={`text-center text-[10px] font-medium py-1 ${i === 6 ? 'text-red-400' : i === 5 ? 'text-blue-400' : 'text-[#78716c]'}`}>{label}</div>
            ))}
          </div>
          {monthLoading ? (
            <div className="text-center py-16">
                  ...
                  {week.map((dateStr, di) => {
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-lg p-1 text-left transition-colors min-h-[60px] ${count > 0 ? 'bg-white shadow-sm hover:shadow-md' : 'bg-[#f5f5f4]/50 hover:bg-white'} ${isToday ? 'ring-2 ring-[#b8860b]/50' : ''}`}
                      >
                        <span className={`text-xs font-medium block ${isToday ? 'text-[#b8860b]' : isSun ? 'text-red-400' : isSat ? 'text-blue-400' : 'text-[#44403c]'}`}>{day}</span>
                        {count > 0 && (
                          <>
          ...
          <div className="mt-6 flex items-center justify-center gap-4 text-[10px] text-[#78716c]">
            <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-white shadow-sm inline-block" /> 出勤あり</span>
            <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-[#f5f5f4]/50 inline-block" /> 出勤なし</span>
            <span className="flex items-center gap-1"><span className="w-3 h-3 rounded ring-2 ring-[#b8860b]/50 inline-block" /> 今日</span>
          </div>
        </div>
      )}
    </main>
